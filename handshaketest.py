...
st.caption("2026 Search: Oracle 23ai TLS + HNSW Indexing + Multi-LLM (Groq, Google, xAI)")
...
with st.sidebar:
    ...
    if any(m in model_choice for m in ["Pro", "Grok", "OSS", "Compound"]):
        st.caption("✨ High-Performance Reasoning Active.")
...
def init_connections(engine_choice):
    try:
        ...
        elif engine_choice == "Grok 3 (Direct xAI)":
            # Grok uses the OpenAI-compatible SDK/format
            llm = ChatOpenAI(
                model="grok-3", 
                openai_api_key=st.secrets["XAI_API_KEY"],
...
if prompt := st.chat_input("Ask about Freddy's experience..."):
    ...
    with st.chat_message("assistant"):
        ...
        INSTRUCTIONS: Use the context to highlight Freddy's technical strengths.
        ANSWER:
        """
