...
with st.sidebar:
    ...
    model_choice = st.selectbox("Select AI Engine:", options=available_models, key="model_v4")
...
# --- 3. Custom Hybrid Search Function ---
def hybrid_search_logic(query, v_store, conn, boost_keywords=True):
    ...
    # This finds exact matches in the 'TEXT' column without needing a special index
    keyword_results = []
    try:
        cursor = conn.cursor()
        # Using simple SQL 'LIKE' for maximum compatibility
        search_term = f"%{query}%"
    ...
    # Merge: Keyword matches go to the top
    return keyword_results + vector_results

# --- 4. Chat Loop ---
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": "I am ready. Ask me anything about Freddy's career!"}]

for msg in st.session_state.messages:
    with st.chat_message(msg["role"]): st.markdown(msg["content"])

if prompt := st.chat_input("Search skills..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing..."):
            ...
            # 2. Generate Answer
            template = "You are Freddy's agent. Use context to answer.\nContext: {c}\nQuestion: {q}"
